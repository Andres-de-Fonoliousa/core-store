...
const content = `// src/Constants.js - PATCHED for auth.bokla.me
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    sftp.writeFile(constantsPath, Buffer.from(content), (err) => {
      if (err) { console.error('Write:', err.message); conn.end(); return; }
      console.log('Patched Constants.js → auth.bokla.me');
      conn.exec('kill $(lsof -t -i:3001) 2>/dev/null; sleep 2; setsid node /var/www/store-app/sham-bridge/server.js < /dev/null > /tmp/sham-bridge.log 2>&1 &', (e2) => {
