...
<template>
  ...
  <div dir="rtl" class="min-h-screen bg-[#050507] text-white relative isolate overflow-x-hidden" style="font-family: 'Cairo', 'Space Grotesk', sans-serif;">
    <!-- Ambient -->
    ...
    <nav class="fixed top-0 left-0 right-0 z-40 transition-all duration-300" :class="scrolled ? 'bg-[#070709]/80 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'">
      <div class="w-full px-4 sm:px-6 lg:px-10 xl:px-16 h-20 flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center gap-8">
          <Link href="/" class="text-sm text-white font-medium relative flex items-center gap-1.5"><Home class="w-4 h-4" /> الرئيسية<span class="absolute -bottom-2 right-0 w-full h-0.5 bg-cyan-400" /></Link>
          <Link href="/products" class="text-sm text-white/60 hover:text-white transition-colors relative group">المنتجات<span class="absolute -bottom-2 right-0 w-0 h-0.5 bg-cyan-400 group-hover:w-full transition-all duration-300" /></Link>
          <Link v-if="authStore.user" href="/orders" class="text-sm text-white/60 hover:text-white transition-colors relative group">طلباتي<span class="absolute -bottom-2 right-0 w-0 h-0.5 bg-cyan-400 group-hover:w-full transition-all duration-300" /></Link>
          <Link v-if="authStore.user" href="/deposit" class="text-sm text-white/60 hover:text-white transition-colors relative group">الإيداع<span class="absolute -bottom-2 right-0 w-0 h-0.5 bg-cyan-400 group-hover:w-full transition-all duration-300" /></Link>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <Link v-if="!authStore.user" href="/login" class="hidden sm:block text-sm text-white/60 hover:text-white transition-colors px-4 py-2">تسجيل الدخول</Link>
          <Link v-if="!authStore.user" href="/register" class="hidden sm:block px-5 py-2 text-sm font-semibold bg-cyan-400 text-black rounded-xl hover:bg-cyan-300 transition-all hover:shadow-[0_0_25px_-5px_rgba(34,211,238,0.5)]">ابدأ الآن</Link>
      ...
      <div v-if="mobileMenu" class="lg:hidden bg-[#070709]/95 backdrop-blur-xl border-t border-white/5">
        <div class="px-6 py-4 flex flex-col gap-1">
          <Link href="/" class="text-sm text-white font-medium py-3 px-3 rounded-lg bg-cyan-500/5 text-cyan-400 flex items-center gap-2"><Home class="w-4 h-4" /> الرئيسية</Link>
          <Link href="/products" class="text-sm text-white/60 hover:text-white py-3 px-3 rounded-lg hover:bg-white/5 transition-colors" @click="mobileMenu = false">المنتجات</Link>
          <Link v-if="authStore.user" href="/orders" class="text-sm text-white/60 hover:text-white py-3 px-3 rounded-lg hover:bg-white/5 transition-colors" @click="mobileMenu = false">طلباتي</Link>
          <Link v-if="authStore.user" href="/deposit" class="text-sm text-white/60 hover:text-white py-3 px-3 rounded-lg hover:bg-white/5 transition-colors" @click="mobileMenu = false">الإيداع</Link>
          ...
          <Link v-if="!authStore.user" href="/login" class="text-center text-sm text-white/60 py-3 px-3 rounded-lg hover:bg-white/5 transition-colors">تسجيل الدخول</Link>
          <Link v-if="!authStore.user" href="/register" class="mt-2 px-6 py-3 text-sm font-semibold bg-cyan-400 text-black rounded-xl text-center">ابدأ الآن</Link>
